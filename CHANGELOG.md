...
- **ec2:** new `azure-vm` provider (`floci.services.ec2.provider=azure-vm`) backs real-mode instances with real Azure virtual machines provisioned through the `az` CLI, keeping the AWS wire protocol unchanged. Instance types map to Azure VM sizes, key pairs and UserData (cloud-init) are injected at create, security-group ingress opens NSG ports, stop/start map to deallocate/start, and terminate deletes the VM with its disk, NIC, NSG, and public IP. Adds `floci.services.ec2.azure-vm` (image, size, admin-username).
